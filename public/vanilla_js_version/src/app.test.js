import {
  createSteeringSeatElement,
  createDrummerSeatElement,
  createSeatElementAtPosition,
  determinePaddlerSeatPosition,
  createPaddlerSeatElements,
  createOnePaddlerSeatElement,
  findPaddlerById,
  getActivePerson,
  deletePersonElementFromBoat,
} from "./app.js";
...
test("check that assert is called when specified paddler is not found from list of paddlers", () => {
  ...
  expect(console.assert).toHaveBeenCalledWith(
    false,
    "Could not find paddler with id %d",
    falsePaddlerId
  );
});
...
test("checks for one paddler seat element in boat", () => {
  ...
  expect(boatElement.innerHTML).toBe(
    '<h1 class="curlyFont">Boat</h1>' +
      '<div class="seat" style="top: 120px; left: 100px;"></div>'
  );
});
...
test("checks for steering element in boat", () => {
  ...
  expect(boatElement.innerHTML).toBe(
    '<h1 class="curlyFont">Boat</h1>' +
      '<div class="seat" style="top: 580px; left: 125px;"></div>'
  );
});
...
test("checks for drummer element in boat", () => {
  ...
  expect(boatElement.innerHTML).toBe(
    '<h1 class="curlyFont">Boat</h1>' +
      '<div class="seat" style="top: 10px; left: 125px;"></div>'
  );
});
...
test("checks for seat element in boat at specified pixel coordinate", () => {
  ...
  expect(boatElement.innerHTML).toBe(
    '<h1 class="curlyFont">Boat</h1>' +
      '<div class="seat" style="top: 1000px; left: 450px;"></div>'
  );
});
